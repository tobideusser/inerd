...
class WNUT17HuggingFaceParser(BaseParser):
    def __init__(
        self,
        ...
        self.dataset_name = dataset_name if dataset_name else "WNUT-17"

    def _parse_wnut17_split(self, dataset: DatasetDict, split_type: str):
        parsed = []
        i = 0
    ...
    def parse(self) -> NERCorpus:
        dataset = load_dataset("wnut_17", cache_dir=self.cache_dir)
        ...
        for split_type in ["train", "validation", "test"]:
            corpus[split_type] = self._parse_wnut17_split(dataset=dataset, split_type=split_type)
